...
st.title("Dental Arch Pipeline 2 Completion")
st.caption(
...
def load_model_weights():
    ...
    try:
        ...
        st.error(f"Model weights are incompatible with the Pipeline 2 model: {error}")
        return None, device
    model.eval()
    return model, device
...
def point_display_name(fdi, point_idx):
    if fdi in ANTERIOR_FDI:
        return "Inner-Mesial midpoint" if point_idx == 0 else "Inner-Distal midpoint"
    if fdi in POSTERIOR_FDI:
...
def proximal_curve_point_order(fdi):
    """Order Mesial/Distal-sided points so adjacent contact sides stay adjacent."""
    quadrant = fdi // 10
    if quadrant in (1, 4):
...
def collect_curve_anchor_points(landmarks, tooth_valid_mask, fdi_order):
    """
    Build ordered Pipeline 2 anchors for curve fitting.

    Both Pipeline 2 points carry Mesial/Distal-side semantics. Point0 is
    Mesial-sided and Point1 is Distal-sided for both raw posterior points and
    derived anterior midpoints.
    """
    anchors = []
    valid_indices = [idx for idx in range(SEQ_LEN) if tooth_valid_mask[idx]]

    for tooth_idx in valid_indices:
        fdi = fdi_order[tooth_idx]
        tooth_points = landmarks[tooth_idx]
        order = proximal_curve_point_order(fdi)
...
with summary_col:
    ...
    st.subheader("Pipeline 2 Points")
    st.markdown(
        "- **Anterior teeth:** Point0 = Inner-Mesial midpoint, Point1 = Inner-Distal midpoint\n"
        "- **Posterior teeth:** Point0 = Mesial, Point1 = Distal\n"
        "- **Curve:** fixed from all ground-truth Pipeline 2 points before masking"
    )
