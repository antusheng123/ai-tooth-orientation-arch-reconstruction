...
SEQ_LEN = 16
LANDMARK_NAMES = ("Mesial", "Distal", "OuterPoint", "InnerPoint")
...
def extract_arch_landmarks(obj_path, label_path, kpt_path, jaw_type):
    """
    Return ordered tooth landmarks with shape (16, 4, 3).

    A tooth is valid only when all four landmark classes are present. If a
    class is annotated more than once, its coordinates are averaged. Teeth
    with incomplete annotations remain zero and are excluded from supervision.
    """
    ...
    valid_fdi = set(UPPER_FDI if jaw_type == 0 else LOWER_FDI)
    ...
    for landmark in landmark_objects:
        ...
        if class_name not in LANDMARK_TO_IDX or coord is None:
            continue
    ...
    for tooth_id, class_points in grouped.items():
        tooth_idx = FDI_TO_IDX[tooth_id]
        for class_name, coords in class_points.items():
            landmark_idx = LANDMARK_TO_IDX[class_name]
            landmarks[tooth_idx, landmark_idx] = np.mean(coords, axis=0)
            landmark_valid_mask[tooth_idx, landmark_idx] = True
...
def landmarks_to_centers(landmarks, tooth_valid_mask=None):
    """Derive one geometric center per tooth from its four ordered landmarks."""
...
class DentalArchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            existing_indices = np.flatnonzero(tooth_valid_mask)
            max_drop = min(5, len(existing_indices) - self.min_remaining_teeth)
            if max_drop >= 1:
                drop_count = random.randint(1, max_drop)
                drop_indices = random.sample(existing_indices.tolist(), drop_count)
