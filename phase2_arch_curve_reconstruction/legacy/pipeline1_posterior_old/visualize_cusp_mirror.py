...
def load_tooth_landmarks(obj_path, label_path, kpt_path, fdi):
    ...
    return tooth_vertices, np.asarray(grouped["OuterPoint"]), np.asarray(grouped["Cusp"])
...
def estimate_lateral_axis(tooth_vertices, buccal_axis):
    centered_vertices = tooth_vertices - tooth_vertices.mean(axis=0)
    projected_vertices = centered_vertices - (
        centered_vertices @ buccal_axis
    )[:, None] * buccal_axis
    if np.linalg.norm(projected_vertices) < 1e-6:
        return None

    covariance = projected_vertices.T @ projected_vertices
    eigenvalues, eigenvectors = np.linalg.eigh(covariance)
    lateral_axis = eigenvectors[:, int(np.argmax(eigenvalues))]
    lateral_axis = lateral_axis - np.dot(lateral_axis, buccal_axis) * buccal_axis
    return normalize_vector(lateral_axis)


def analyze_geometry(tooth_vertices, outer_points, cusp_points):
    ...
    lateral_axis = estimate_lateral_axis(tooth_vertices, buccal_axis)
    if lateral_axis is None:
        raise ValueError("Cannot estimate lateral axis from tooth mesh geometry")
...
def add_mirror_plane(fig, center, lateral_axis, buccal_axis, vertices):
    ...
    fig.add_trace(
        go.Surface(
            ...
            showscale=False,
            name="mirror plane",
            hovertemplate="Mirror plane<br>through tooth center<br>normal = lateral axis<extra></extra>",
        )
    )


def build_figure(tooth_vertices, outer_points, cusp_points, analysis, fdi):
    fig = go.Figure()
    ...
    add_vector(fig, center, lateral_axis, "lateral axis: mirror normal", "#EAB308", axis_scale)

    fig.update_layout(
        title=(
            f"FDI {fdi} cusp mirror geometry<br>"
            "Plane: through tooth center, normal = lateral axis. "
...
def main():
    ...
    tooth_vertices, outer_points, cusp_points = load_tooth_landmarks(
        args.obj, args.label, args.kpt, args.fdi
    )
    analysis = analyze_geometry(tooth_vertices, outer_points, cusp_points)
    fig = build_figure(tooth_vertices, outer_points, cusp_points, analysis, args.fdi)
    ...
    print("Lateral axis:", np.round(analysis["lateral_axis"], 4).tolist())
