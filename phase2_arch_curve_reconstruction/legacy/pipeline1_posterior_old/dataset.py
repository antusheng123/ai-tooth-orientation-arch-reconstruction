...
def _estimate_lateral_axis(tooth_vertices, buccal_axis):
    """Estimate the tooth's left-right axis in the plane perpendicular to buccal."""
...
def _mirror_single_buccal_cusp(cusp_point, outer_point, tooth_vertices):
    """
    Synthesize a second buccal cusp by mirroring one annotated cusp.

    The mirror plane passes through the tooth mesh center. Its normal is a
    mesh-derived lateral axis, estimated after removing the buccal direction
    defined by OuterPoint. The generated point keeps the original cusp's
    buccal/depth component and flips only the lateral component.
    """
    tooth_vertices = np.asarray(tooth_vertices, dtype=np.float32)
    ...
    lateral_axis = _estimate_lateral_axis(tooth_vertices, buccal_axis)
...
def extract_arch_landmarks(obj_path, label_path, kpt_path, jaw_type):
    ...
    tooth mesh geometry. Teeth that still cannot produce two points remain
