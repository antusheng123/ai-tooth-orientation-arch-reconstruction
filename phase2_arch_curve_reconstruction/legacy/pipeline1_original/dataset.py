...
def extract_arch_landmarks(obj_path, label_path, kpt_path, jaw_type):
    ...
    Anterior teeth use Mesial and Distal. Posterior teeth use the two Cusp
    points closest to OuterPoint, which acts as the buccal-side reference.
    Teeth with incomplete annotations remain zero and are excluded from
    supervision.
    """
    ...
    for tooth_id in ordered_fdi:
        ...
        if tooth_id in ANTERIOR_FDI:
            ...
            if "OuterPoint" not in class_points or len(class_points.get("Cusp", [])) < 2:
                continue
            outer_point = np.mean(class_points["OuterPoint"], axis=0)
            selected_cusps = _select_reliable_buccal_cusps(
                class_points["Cusp"],
                outer_point,
                tooth_vertices_by_fdi.get(tooth_id, np.empty((0, 3))),
            )
