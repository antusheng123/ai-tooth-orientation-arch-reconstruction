...
def masked_landmark_loss(predictions, targets, tooth_mask):
    """Combine point, derived-midpoint, and intra-tooth geometry losses."""
    ...
    pred_distances = torch.linalg.vector_norm(
        pred_teeth[:, 0] - pred_teeth[:, 1], dim=-1
    )
    target_distances = torch.linalg.vector_norm(
        target_teeth[:, 0] - target_teeth[:, 1], dim=-1
    )
...
def make_masked_validation_batch(batch, device, batch_index, seed=1042):
    ...
    for row in range(tooth_valid_mask.shape[0]):
        sampled = sample_consecutive_valid_teeth(
            tooth_valid_mask[row].numpy(),
            min_drop=3,
            max_drop=6,
            min_remaining_teeth=4,
            rng=rng,
        )
        dropped_mask[row] = torch.from_numpy(sampled)
...
def main():
    ...
    print("Loading Pipeline 1 two-point arch data...")
